...
export async function createGoogleCalendarEvent({
    ...
    try {
        ...
        if (!oauth2Client) {
            console.warn(`Skipping Google Calendar event creation for user ${userId}: No connected account`);
            return;
        }
        ...
        // We throw here or swallow? The plan said "Log error but do not fail the job"
        // But throwing allows the caller (bookings queue) to decide. 
        // The plan specifically said "If Calendar fails: Log error but do not fail the job."
        // I'll swallow the error here to be safe, or let the queue handle it? 
        // The queue might want to know if it success/fail for logging but continue.
        // I will throw so the queue knows, but catch it in the queue.
        throw error;
    }
}
